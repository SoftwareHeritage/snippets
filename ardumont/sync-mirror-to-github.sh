...
FORGE_REPO_URL=$2
FORGE_REPO_DESCRIPTION=$3
CREDENTIAL_KEY_ID=${4-"2"}  # default to K2
...
FORGE_REPO_URLS=$(echo $DATA_REPO | jq '.result.data[].attachments.uris.uris[].fields.uri.display' \
                      | sed -e 's/"//gi')

already_mirrored=$(echo $FORGE_REPO_URLS | grep 'github')

if [ ! -z "$already_mirrored" ]; then
    echo "Repository $FORGE_REPO_CALLSIGN already mirrored. Skipping."
    exit 1
fi

# Retrieve name from url
mirror_url=$(echo $DATA_REPO | jq '.result.data[].attachments.uris.uris[].fields.uri.display' \
                 | sed -e 's/"//gi' | head -1 | xargs basename)

repo_name=${mirror_url%%.git}
...
GITHUB_REPO_URI=git@github.com:SoftwareHeritage/$repo_name.git
